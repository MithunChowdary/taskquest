            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {notes.length > 0 ? (
                    notes.map(note => (
                        <div
                            key={note._id}
                            className={`group p-6 rounded-[2.5rem] border backdrop-blur-md transition-all hover:scale-[1.02] hover:-rotate-1 relative overflow-hidden break-words ${theme === 'dark'
                    ...
                    <div className="col-span-full flex flex-col items-center justify-center py-24 text-slate-400">
                        <div className={`w-24 h-24 rounded-full flex items-center justify-center mb-6 ${theme === 'dark' ? 'bg-slate-800/20' : 'bg-orange-100/50'}`}>
                            <svg className={`w-12 h-12 opacity-30 ${theme === 'dark' ? 'text-white' : 'text-orange-500'}`} fill="currentColor" viewBox="0 0 20 20"><path d="M11 3a1 1 0 10-2 0v1a1 1 0 102 0V3zM15.657 5.757a1 1 0 00-1.414-1.414l-.707.707a1 1 0 001.414 1.414l.707-.707zM18 10a1 1 0 01-1 1h-1a1 1 0 110-2h1a1 1 0 011 1zM5.05 6.464A1 1 0 106.464 5.05l-.707-.707a1 1 0 00-1.414 1.414l.707.707zM5 10a1 1 0 01-1 1H3a1 1 0 110-2h1a1 1 0 011 1zM8 16v-1h4v1a2 2 0 11-4 0zM12 14c.015-.34.208-.646.477-.859a4 4 0 10-4.954 0c.27.213.462.519.477.859h4z" /></svg>
                        </div>
                        <p className={`text-2xl font-black uppercase italic tracking-widest opacity-40 ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>No Knowledge Stored</p>
                    </div>
                )}
            </div>
        </div>
